...
namespace JulMar.Windows.Undo
{
    /// <summary>
    /// Class used for Undo/Redo support.
    /// </summary>
    [ExportServiceProvider(typeof(IUndoService))]
    public class UndoManager : IUndoService
    {
        ...
        public UndoManager(int maxSupported)
        {
            ...
            // Register two command handlers for Undo/Redo support by WPF
            CommandManager.RegisterClassCommandBinding(typeof(Window), new CommandBinding(ApplicationCommands.Undo, (s,e) => this.Undo(), (s,e) => e.CanExecute = this.CanUndo));
            CommandManager.RegisterClassCommandBinding(typeof(Window), new CommandBinding(ApplicationCommands.Redo, (s, e) => this.Redo(), (s, e) => e.CanExecute = this.CanRedo));
        }
