...
namespace JulMar.Windows
{
    ...
    /// </summary>
    public class CollectionObserver : IDisposable, IWeakEventListener
    {
        ...
        public CollectionObserver(object collection)
        {
            if (collection == null)
                throw new ArgumentNullException("collection");

            INotifyCollectionChanged ncc = collection as INotifyCollectionChanged;
            if (ncc == null)
                throw new ArgumentException("Collection must implement INotifyCollectionChanged.");

            _source = new WeakReference(collection);
            CollectionChangedEventManager.AddListener(ncc, this);
        }
