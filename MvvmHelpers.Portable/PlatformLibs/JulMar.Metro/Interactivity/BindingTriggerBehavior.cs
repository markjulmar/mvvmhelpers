...
namespace JulMar.Interactivity
{
    ...
    public sealed class BindingTriggerBehavior : DependencyObject, IBehavior
    {
        ...
        private void EvaluateBindingChange(DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!hasValue || ComparisonLogic.Evaluate(this.Binding, this.ComparisonCondition, this.Value))
            {
                // Pass the result of the binding
                Interaction.ExecuteActions(this.AssociatedObject, this.Actions, this.Binding);
            }
        }
