...
namespace JulMar.Interactivity
{
    ...
    public class ViewModelTriggerBehavior : DependencyObject, IBehavior
    {
        ...
        {
            EventInfo eventInfo = this.LookForEventDeclaration(obj, eventName);
            if (eventInfo == null)
                throw new ArgumentException("Cannot find event " + eventName);
            var handler = this.GetEventMethod(eventInfo);
        ...
        {
            Interaction.ExecuteActions(this.AssociatedObject, this.Actions, parameter);
        }
        ...
        public void Attach(DependencyObject associatedObject)
        {
            if ((associatedObject != this.AssociatedObject) && !DesignMode.DesignModeEnabled)
            {
                ...
                if (this.Target != null && !string.IsNullOrEmpty(this.EventName))
                    this.RegisterEvent(this.Target, this.EventName);
            }
        }
        ...
        {
            if (this.Target != null && !string.IsNullOrEmpty(this.EventName))
                this.UnregisterEvent(this.Target, this.EventName);
