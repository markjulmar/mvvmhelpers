...
namespace JulMar.Interactivity.Input
{
    /// <summary>
    ...
    public class KeyTriggerBehavior : DependencyObject, IBehavior
    {
        ...
        {
            if (e.VirtualKey == this.Key && this.IsModifierKeyDown())
            {
        ...
        private bool IsModifierKeyDown()
        {
            ...
            {
                if (this._targetElement.GetKeyState(VirtualKey.LeftWindows) != CoreVirtualKeyStates.Down
                    && this._targetElement.GetKeyState(VirtualKey.RightWindows) != CoreVirtualKeyStates.Down)
                    good = false;
            }

            if ((this.Modifiers & ModifierKeys.Shift) == ModifierKeys.Shift)
            {
                if (this._targetElement.GetKeyState(VirtualKey.LeftShift) != CoreVirtualKeyStates.Down
                    && this._targetElement.GetKeyState(VirtualKey.RightShift) != CoreVirtualKeyStates.Down)
                    good = false;
            }

            if ((this.Modifiers & ModifierKeys.Control) == ModifierKeys.Control)
            {
                if (this._targetElement.GetKeyState(VirtualKey.Control) != CoreVirtualKeyStates.Down)
                    good = false;
            }

            if ((this.Modifiers & ModifierKeys.Menu) == ModifierKeys.Menu)
            {
                if (this._targetElement.GetKeyState(VirtualKey.Menu) != CoreVirtualKeyStates.Down)
                    good = false;
            }
