...
namespace JulMar.Interactivity
{
    ...
    public class DataGridDragRowBehavior : Behavior<DataGrid>
    {
        ...
        protected override void OnAttached()
        {
            ...
            this.AssociatedObject.DragEnter += OnCheckDropTarget;
            this.AssociatedObject.DragOver += OnCheckDropTarget;
            this.AssociatedObject.DragLeave += OnCheckDropTarget;
        ...
        {
            IList collection = (IList)this.AssociatedObject.ItemsSource ?? this.AssociatedObject.Items;
            ...
            if (e.LeftButton == MouseButtonState.Pressed)
            {
                ...
                if ((row != null) && row.IsSelected)
                {
                    ...
                    if ((finalDropEffect == DragDropEffects.Move) && (this._target != null))
                    {
                        ...
                        int newIndex = collection.IndexOf(this._target);
                        if (newIndex == -1 && this._target == CollectionView.NewItemPlaceholder)
                            newIndex = collection.Count - 1;

                        if (oldIndex != newIndex && oldIndex >= 0 && newIndex >= 0)
                        {
                            var dropStarted = this.DropStarted;
                            if (dropStarted != null)
                            ...
                            var dropFinished = this.DropFinished;
