...
namespace JulMar.Interactivity
{
    ...
    public class SynchronizedScrollingBehavior : Behavior<UIElement>
    {
        ...
        {
            this.AssociatedObject.AddHandler(ScrollViewer.ScrollChangedEvent, new ScrollChangedEventHandler(this.OnSourceScroll));
            base.OnAttached();
        }
        ...
        {
            this.AssociatedObject.RemoveHandler(ScrollViewer.ScrollChangedEvent, new ScrollChangedEventHandler(this.OnSourceScroll));
            base.OnDetaching();
        }
        ...
        private void OnSourceScroll(object sender, ScrollChangedEventArgs e)
        {
            if (this.Target != null)
            {
                ScrollViewer sv = this.Target as ScrollViewer ?? this.Target.FindVisualChild<ScrollViewer>();
                if (sv != null)
                    AdjustScrollPosition(sv, e, -1*this.HorizontalAdjustment, -1*this.VerticalAdjustment);
            }
        }
        ...
        {
            ScrollViewer sv = this.AssociatedObject as ScrollViewer ?? this.AssociatedObject.FindVisualChild<ScrollViewer>();
            if (sv != null)
                AdjustScrollPosition(sv, e, this.HorizontalAdjustment, this.VerticalAdjustment);
        }
