...
namespace JulMar
{
    ...
    public class OperatingSystemInfo
    {
        ...
        public OperatingSystem Product
        {
            get
            {
                if (this._os == OperatingSystem.Unknown)
                    this._os = DetermineOperatingSystem();
                return this._os;
            }
        }
            ...
            get
            {
                if (this.isEmbedded == null)
                {
                    ...
                    if (NativeMethods.GetVersionEx(ref osvi))
                        this.isEmbedded = (osvi.wSuiteMask & 0x40)==0x40;  // VER_SUITE_EMBEDDEDNT
                }
                return this.isEmbedded.Value;
            }
        }
        ...
        {
            return string.Format("{0} {1} {2}", this.Product, this.ServicePack, this.ProductInfo);
        }
            ...
            get
            {
                if (this._prodInfo == null)
                {
                    if (Environment.OSVersion.Version.Major >= 6)
                        this._prodInfo = GetProductInfo();
                    else
                    {
                        if (this.IsMediaCenter)
                            this._prodInfo = "Media Center";
                        else if (this.HasTabletSupport)
                            this._prodInfo = "Tablet";
                        else if (this.IsEmbedded)
                            this._prodInfo = "Embedded";
                    }
                }
