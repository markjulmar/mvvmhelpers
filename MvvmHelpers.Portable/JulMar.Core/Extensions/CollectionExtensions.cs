                ...
                for (int child = 1; child <= index; child++)
                {
                    ...
                    if (comparer == null)
                    {
                        if (d1.GetType() == d2.GetType())
                        {
                            ...
                            var property = comparerType.GetTypeInfo().GetDeclaredProperty("Default");
                            if (property != null) comparer = (IComparer)property.GetValue(null);
                        }
                    }

                    if (comparer == null)
                        throw new InvalidOperationException(string.Format(
                            "No Comparer found for {0} to {1}, pass in an explicit comparer.", d1.GetType().Name, d2.GetType().Name));
