...
namespace JulMar.Core.Services
{
    ...
    public sealed class MessageMediator : IMessageMediator
    {
        ...
        public void Register(object view)
        {
            ...
            foreach (var mi in view.GetType().GetTypeInfo().DeclaredMethods)
            {
                ...
                foreach (var att in mi.GetCustomAttributes(typeof(MessageMediatorTargetAttribute), true))
                {
                    ...
                    InternalRegisterHandler(key, 
                                    mi.IsStatic
                                        ? mi.CreateDelegate(actionType)
                                        : mi.CreateDelegate(actionType, view));
                }
            }
        }
        ...
        {
            InternalRegisterHandler(key, handler);
        }
        ...
        {
            InternalRegisterHandler(typeof(Action<T>), handler);
        }
        ...
        {
            InternalUnregisterHandler(key, handler);
        }
        ...
        {
            InternalUnregisterHandler(typeof(Action<T>), handler);
        }
        ...
        private void InternalRegisterHandler(object key, Delegate handler)
        {
        ...
        /// <param name="key">Key to store in dictionary</param>
        /// <param name="handler">Method</param>
        private void InternalUnregisterHandler(object key, Delegate handler)
        {
            Type actionType = handler.GetType();
            lock (_registeredHandlers)
            {
        ...
        public bool SendMessage<T>(T message)
        {
            ...
            lock (_registeredHandlers)
            {
                keyList = _registeredHandlers.Keys.Where(key => key is Type && ( ((Type)key).GetTypeInfo().IsAssignableFrom(actionType.GetTypeInfo()) 
