...
namespace JulMar.Windows.Mvvm
{
    ...
    public sealed class AsyncDelegateCommand : IDelegateCommand
    {
        ...
        {
            return (_canExecute == null) ? !_isBusy : !_isBusy && _canExecute(parameter);
        }
        ...
        public void Execute(object parameter)
        {
            ...
            // Spin up the task.
            Task parentTask = Task.Factory.StartNew(_execute, parameter, CancellationToken.None,
                TaskCreationOptions.LongRunning, TaskScheduler.Default);

            // Do the cleanup on the UI thread.
            parentTask.ContinueWith(t =>
            {
                if (_completedCallback != null && t.Exception == null)
                    _completedCallback();
                if (_errorCallback != null && t.Exception != null)
                    _errorCallback(t.Exception);
    ...
    public sealed class AsyncDelegateCommand<T> : IDelegateCommand
    {
        ...
        public bool CanExecute(object parameter)
        {
            return (_canExecute == null) ? !_isBusy : !_isBusy && _canExecute((T)parameter);
        }
        ...
        public void Execute(object parameter)
        {
            ...
            Task parentTask = Task.Factory.StartNew(o => _execute((T)o), parameter, CancellationToken.None,
                TaskCreationOptions.LongRunning, TaskScheduler.Default);
            parentTask.ContinueWith(t =>
            {
                if (_completedCallback != null && t.Exception == null)
